...
`pnpm desktop:qvac` runs that same two-window flow with **real QVAC models** — actual GTE embeddings for retrieval and actual Qwen generation for summary drafts — behind the simulated transport. The transport stays simulated, so this demonstrates real local inference but not cross-device delivery; the window header and the page notice both say so, and `DesktopInfo.aiProvider` reports `qvac` separately from `mode`. A stored index is bound to its embedding profile, so this mode keeps its own workspace directory (`core-simulated/AB-qvac`) and never mixes with the simulated-AI index. The first run downloads roughly 670 MB (GTE) and 1.1 GB (Qwen) into the QVAC cache, and the first question takes noticeably longer than the simulated path while the models load. A failed SDK load stops startup; it never falls back to a fake port.
