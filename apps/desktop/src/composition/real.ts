...
export function readRealConfiguration(raw: unknown): RealConfiguration {
  ...
  if (Object.keys(record).some(k => !['bootstrap', 'localPort'].includes(k)) || !Array.isArray(record.bootstrap) || !record.bootstrap.length || record.bootstrap.length > 16) throw new KuroError('INVALID_INPUT');
  ...
  return { bootstrap, ...(record.localPort === undefined ? {} : { localPort: record.localPort as number }) };
}

export async function createRealDesktop(directory: string, profile: 'A' | 'B', secretStore: SecretStore, configuration: RealConfiguration) {
