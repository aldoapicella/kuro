...
1. **Electron lifecycle:** Electron 44.3.0 now passes the packaged SQLite/identity probes and the separate actual cross-process protected-store contention check described in D29. The remaining host gate is a trustworthy synchronous clock/lifecycle epoch followed by actual suspend/resume validation. The real composition remains closed at startup and resumes with `false`; delayed power notifications do not justify reopening. Other OS protected-storage backends remain additional platform checks.
