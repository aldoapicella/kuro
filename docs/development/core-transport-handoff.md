...
| 10 | Real Bare HyperDHT workers exchange authenticated D25 messages through independent Node hosts. Desktop native packaging and physical offline LAN remain unverified. |
...
3. **Desktop native packaging:** the Bare 1.32.0 worker and Node bridge now run the shared conformance suite and combined real QVAC workflow (D28). Package the built worker, Bare binary and pinned native dependencies in the selected Electron distribution, supply its trusted worker asset URL and repeat the same checks in that distributed application. No Electron executable/build is currently supplied.
