...
Participant caches are stale after restart. High-water marks, accepted bytes and deduplication survive; outstanding request correlations do not cross process/lifecycle epochs. D25 validity is at most 900,000 ms from the original request send with both wall and monotonic deadlines, not response arrival. A same-policy renewal before expiry preserves work; a renewal after an unnoticed lapse first invalidates expired work. Denial and policy change cannot revive cancelled approvals.
...
| Core tests | 55 passed, including 26 authority cases and six separate-process SIGKILL cases. |
| Transport tests | 7 passed. |
...
| SQLite host probe | Node 24.19.0 / SQLite 3.53.3, FTS5, foreign keys and persistent reopen passed. |

Initial validation exposed and corrected D25 parser round trips, lifecycle/expiry renewal races, interrupted-job state reconciliation, selected-file replacement boundaries, summary manifest rollback, and transport startup cancellation. The final runs above have no failing or skipped tests. Their scope does not include the external gates below.
...
| B05 | `lifecycle.test.ts`, `workflow.test.ts`: source change/revocation before dispatch, stale review rejection, lost ACK and exact persisted-byte retry. |
...
1. **Electron host:** provide the selected Electron version/build. Run `scripts/probe-host.mjs` in that application's main-process runtime and record `process.versions`, SQLite, FTS5, foreign keys and disk reopen. Repeat the core lifecycle tests through the actual host suspend/resume barrier. Verify the OS secret adapter reports protected storage and rejects unavailable/Linux `basic_text`. Terminal Node alone cannot close this gate.
