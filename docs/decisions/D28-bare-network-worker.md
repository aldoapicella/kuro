...
A subsequent test-only check also passed: losing the inherited host pipe before or during network startup exits the actual Bare process without timeout or orphaning. This brings the transport suite to 23 tests, 129 TypeScript tests across the workspace, with no product-code changes after `84c1de7`.
