...
type VmConfig = { bootstrapHost: string; sshConfig: string; sshHost: string; guestRepository: string; guestStateRoot: string; hostStateRoot?: string; guestPort?: number };
type PeerOutput = { type?: string; requestId?: string; publicKey?: string; result?: Result<unknown>; selectionId?: string; diagnostics?: { records: Array<{ direction: string; type: string; digest: string }>; droppedAcks: number; lifecycle: string[] }; stopped?: boolean; aiCalls?: string[]; error?: string };
...
class Peer {
  ...
  async ready(): Promise<string> { const event = await this.wait('ready'); if (!event.publicKey) throw new Error(`${this.label} has no public key`); return event.publicKey; }
...
function config(): VmConfig { const raw = process.env.KURO_VM_TEST_CONFIG; if (!raw) throw new Error('KURO_VM_TEST_CONFIG is required'); const value = JSON.parse(raw) as Partial<VmConfig>; for (const key of ['bootstrapHost', 'sshConfig', 'sshHost', 'guestRepository', 'guestStateRoot'] as const) if (!value[key]) throw new Error(`${key} is required`); return value as VmConfig; }
...
async function diagnostics(peer: Peer) { const out = await peer.command({ requestId: randomUUID(), action: 'diagnostics' }); if (!out.diagnostics) throw new Error('missing diagnostics'); return { ...out.diagnostics, aiCalls: out.aiCalls ?? [] }; }
...
async function main(): Promise<void> {
  const vm = config(); const run = randomUUID(); const hostState = vm.hostStateRoot ? join(resolve(vm.hostStateRoot), `virtual-core-${run}`) : mkdtempSync(join(tmpdir(), 'kuro-virtual-core-'));
  ...
  const hostConfig = { stateDirectory: hostState, seedHex: ownerSeed.toString('hex'), pairedPeers: [requesterKey], bootstrap, memberId: OWNER, localPort: await reservePort() };
  const guestConfig = { stateDirectory: `${vm.guestStateRoot}/virtual-core-${run}`, seedHex: requesterSeed.toString('hex'), pairedPeers: [ownerKey], bootstrap, memberId: REQUESTER, localPort: vm.guestPort ?? 49747 };
  ...
  const remoteStart = () => spawn('ssh', ['-F', vm.sshConfig, '-o', 'ControlMaster=no', '-o', 'ControlPath=none', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=10', vm.sshHost, `export PATH="$HOME/.local/bin:$PATH"; cd ${shell(vm.guestRepository)} && env KURO_CORE_PEER_CONFIG=${shell(JSON.stringify(guestConfig))} node --import tsx harnesses/core/src/network-peer.ts`], { stdio: ['pipe', 'pipe', 'pipe'] });
  let guest = new Peer(remoteStart, 'vm');
  ...
  const until = async <T>(probe: () => Promise<T>, accept: (value: T) => boolean, label: string): Promise<T> => {
    const deadline = Date.now() + 45_000;
    while (Date.now() < deadline) { const value = await probe(); if (accept(value)) return value; await pump(1); }
    throw new Error(`Timed out waiting for ${label}`);
  };
  try {
    const [actualOwnerKey, actualRequesterKey] = await Promise.all([host.ready(), guest.ready()]); if (actualOwnerKey !== ownerKey || actualRequesterKey !== requesterKey) throw new Error('seed-derived identity mismatch');
    ...
    const importText = async (target: string, text: string, documentRules: typeof rules) => { const selectionId = await select(host, text); await app(host, 'importText', { spaceId: target, selectionId, replaceDocumentId: null, expectedRevision: null, rules: documentRules }); await pump(); };
    ...
    let reviews = await until(() => app(host, 'listReviews', { spaceId }), views => views.length === 1, 'authorized review'); if (reviews.length !== 1 || reviews[0]!.passages.length !== 1 || reviews[0]!.passages[0]!.text !== 'Outstanding observations remain provisional.') throw new Error('forbidden or cross-space text reached review');
    ...
    output({ phase: 'complete', run, stateDirectories: { host: hostState, guest: guestConfig.stateDirectory }, banner: 'AUTOMATED SYNTHETIC TEST only: real HyperDHT framing, FakeAiPort; not QVAC or physical-device validation.' });
  } catch (error) {
