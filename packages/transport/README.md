...
Status: implemented with a Node 24 host and Bare 1.32.0 network worker. `HyperDhtTransport` runs the DHT in a bounded Bare process, derives its identity from a protected 32-byte seed, admits only explicitly paired public keys, and delivers exact validated frames with `socket.remotePublicKey` as identity. The worker has no corpus, SQLite, or policy dependency.
...
Run `pnpm --filter @kuro/transport test` for framing, secret-store and memory conformance checks. Run `pnpm --filter @kuro/transport-harness smoke` for the two-process host / actual Bare HyperDHT loopback check after workspace dependencies are installed.
...
The default worker is built with `pnpm --filter @kuro/transport build:worker`; transport tests and harness scripts build it automatically. It uses pinned `bare-runtime`, a private inherited pipe, and strict UTF-8 validation through the unchanged public contracts. A clean stop requires an acknowledgment and exit zero; timeout termination is a failure. There is no Node-worker fallback. See [D28](../../docs/decisions/D28-bare-network-worker.md) for runtime, IPC bounds, dependencies and validation.

Desktop packaging must include the built worker, Bare binary and native dependencies and supply the trusted host-only `workerUrl` option when assets move. Direct harness entry points require a prior build on every host. Electron composition and physical two-device offline LAN remain unverified; a Bare loopback pass does not close those gates.
